...
En oikeastaan löytänyt mitään suoraan todistettavaa haavoittuvuutta, mutta löytyi ohjelmiston toimintoja ja merkkijonoja, jotka voisivat olla tietoturvan kannalta hyödynnettävissä.  
Ohjelmiston salauksen osalta huomasin, että ohjelmisto voidaan purkaa julkisesti saatavilla olevan TP-Linkin GPL-materiaalin avulla jonka ansiosta salaukseen käytettyyn avaimeen pääsee käsiksi. Tämä ei nyt suoraan tarkoita sitä, että kamera olisi heikosti suojattu, mutta se mahdollistaa kenen tahansa purkaa ja analysoida ohjelmiston sisältöä, mikä madaltaa kynnystä löytää muita mahdollisesti hyödynnettäviä haavoittuvuuksia.  

**bin/main**-binäärin strings-analyysi paljasti merkkijonoja, jotka viittaavat root-salasanan käsittelyyn ja mahdolliseen selväkieliseen tulostukseen lokiin. Nämä olivat **gen_root_passwd**, **root_passwd:%s** ja **factory_passwd**. Itse en nyt päässyt varsinaisesti todistamaan näiden haavoittuvuuksia, mutta nämä voivat olla mahdollisia tietoturvariskejä jos on oikeat työkalut ja taidot saatavilla.  
**getP2PSharePassword** ja **getAESEncryptKey** voisivat olla myös hyödynnettävissä koska vaikuttaa siltä että niiden tarkoitus olisi salasanan tai salausavaimen hakeminen.  
En kuitenkaan oman tutkimisen perusteella voinut todistaa näiden varsinaista haavoittuvuutta. **strings**- ja **grep**-komennot kertovat että kyseiset merkkijonot ovat ohjelmistossa, mutta niiden perusteella en voinut selvittää millä tavalla toimintoja voi käyttää tai hyödyntää.  
Löytyi myös MD5 merkkijono, joka on jo kryptografisesti vanhentunut menetelmä mitä voitaisiin käyttää hyödyksi jos se on kamerassa oikeasti käytössä.  
